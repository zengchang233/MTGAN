...
class OnlineTripletLoss(nn.Module):
    ...
    def forward(self, embeddings, target):
        ...
        ap_distances = F.cosine_similarity(embeddings[triplets[:,0]], embeddings[triplets[:,1]])
        an_distances = F.cosine_similarity(embeddings[triplets[:,0]], embeddings[triplets[:,2]])
...
class Encoder(nn.Module):#定义resnet
    def __init__(self, layers = [1,1,1], block=BasicBlock, embedding_size=None):#block类型，embedding大小，分类数，maigin大小
